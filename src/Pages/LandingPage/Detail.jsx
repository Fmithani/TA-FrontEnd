...
import { NavigateBack, DistributeURIPath } from '../../Helpers';
...
const Detail = () => {
    ...
    // Dummy json response dont use this
    const box = [{
    ...
    const MouseHover = (condition, index) => {
        if (index === 1) setBorderColor1(`${condition}`);
        else setBorderColor(`${condition}`);
    }

    const getBorderColour = (index) => index === 1 ? borderColor1 : borderColor;
                                            ...
                                            <Row>
                                                <Col sm="11" className="relative" style={{ overflow: "visible" }}>
                                                    <div className="relative_pos_for_bracket  ">
                                                        <img alt="images" src={DistributeURIPath(y.url)} className="img-fluid img_of_text_1  "></img>
                                                        {y.labels && y.labels &&
                                                            Object.values(y.labels).map((x, i) =>
                                                                x.Instances && x.Instances.length > 0 ?
                                                                    x.Instances.map((m, q) =>
                                                                        <span
                                                                            key={q + 1}
                                                                            className={getBorderColour(j) === 'All' || getBorderColour(j) === `${i + 1}` ? "text_change" : "text"}
                                                                            style={{ width: `${m.BoundingBox.Width * multipleW}${useDigitalUnit}`, height: `${m.BoundingBox.Height * multipleH}${useDigitalUnit}`, top: `${m.BoundingBox.Top * multipleT}${useDigitalUnit}`, left: `${m.BoundingBox.Left * multipleL}${useDigitalUnit}` }}
                                                                            onMouseEnter={() => MouseHover(i + 1, j)} onMouseLeave={() => MouseHover('', j)}
                                                                        ></span>
                                                                    ) : ''
                                                            )}
                                                    ...
                                                    <div className="txt_cont_on_img_click">
                                                        <div className="p_r_for_bord_img_s">
                                                            <p className=" text-white bor_bott_for_dot_text_img_det woman_txt fs_13 mt-4 mb-3"><span><BsSquare className={`${getBorderColour(j) === 'All' || getBorderColour(j) !== '' ? "text_col_blue" : "colr_white"} blue_icon_for`} onMouseEnter={() => MouseHover('All', j)} onMouseLeave={() => MouseHover('', j)} /> OBJECTS 20.00%</span></p>
                                                            {y.labels && y.labels &&
                                                                Object.keys(y.labels).map((x, i) =>
                                                                    <pre key={i + 1} onMouseEnter={() => MouseHover(i + 1, j)} onMouseLeave={() => MouseHover('', j)} className={`${getBorderColour(j) === 'All' || getBorderColour(j) === `${i + 1}` ? "text_col_blue" : "text-white"} fs_13 mt-3`}><span>{i + 1} {x}</span></pre>
                                                                )}
                                                        </div>
